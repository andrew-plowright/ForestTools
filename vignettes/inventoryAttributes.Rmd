...
We will now apply the newly created `topHgtFun` to our `quesnelTrees` dataset using the `SpatialStatistics` function. We set the `grid` argument to 100 to generate a 100 m x 100 m (1 ha) grid. It is important to enter `topHgtFun` as a _named list_: "Top100" will be part of the name given to the product of the function. If other statistics are required, their corresponding functions can be added to this list.
...
The product of `SpatialStatistics` is a [RasterBrick](https://cran.r-project.org/package=raster/raster.pdf#page=36), i.e.: a multi-layered raster. It contains two layers: _TreeCount_ and _heightTop100_, the product of our custom function when applied to the _height_ attribute of the trees in each cell. We can subset a single layer from the RasterBrick using the `[[]]` operator
