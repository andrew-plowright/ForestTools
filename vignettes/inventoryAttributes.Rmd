...
2. The average height of the 100 tallest trees is calculated for each 1 ha cell on a continuous grid. This metric will be referred to as _top height_.
3. The average 1 ha top height is upscaled a second time to a set of polygonal management units (blocks).
...
Begin by attaching the Forest Tools library and sample data. The sample data is derived from a photogrammetric survey of a 125 ha area in the Quesnel Timber Supply Area in British Columbia, Canada.
...
Given the large number of treetops in the `quesnelTrees` dataset, visualizing it in its entirety is difficult. Instead, it is more practical to view a rasterized representation of the trees' attributes. This can be done using the `SpatialStatistics` function.
...
We will now apply the newly created `topHgtFun` to our `quesnelTrees` dataset using the `SpatialStatistics` function. We set the `grid` argument to 100 to generate a 100 m x 100 m (1 ha) grid. We enter `topHgtFun` as a _named list_: the "Top100" will be the name given to the product of the function.
...
We can see that the product of `SpatialStatistics` is a [RasterBrick](https://cran.r-project.org/package=raster/raster.pdf#page=36), i.e.: a multi-layered raster. `SpatialStatistics` automatically generates a tree count in addition to any custom-defined statistics. We can subset a single layer from the RasterBrick using the `[[]]` operator
...
## Get average top height for polygons

Once our raster-based statistic has been generated, there are a variety of ways to summarize the raster to a set of polygons. In the following example, the `rasterize` and `zonal` functions from the [raster]( https://cran.r-project.org/package=raster) package are used.
