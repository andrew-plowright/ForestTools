...
The `ttops` object created by `TreeTopFinder` in this example contains the spatial coordinates of each detected treetop, as well as two default attributes: _height_ and _winRadius_. These correspond to the tree's height above ground and the radius of the moving window where the tree was located. Note that _winRadius_ **is not necessarily equivalent to the tree's crown radius**.
...
By default, `SegmentCrowns` returns a raster, where each crown is given a unique cell value. Depending on the intended purpose of the crown map, it may be preferable to store these outlines as polygons. Setting the `format` argument to "polygons" will convert the rasterized crown map to a set of polygons (a [SpatialPolygonsDataFrame](https://cran.r-project.org/package=sp/sp.pdf#page=89)). As an added benefit, these polygons will inherit the attributes of the treetops from which they were generated, such as _height_. Furthermore, an extra attribute, _crownArea_, will be calculated for each polygon.

It should be noted, however, that producing crown outlines as polygons requires significantly more processing time and disk space. It is advised to generate crown maps as rasters first in order to quickly visualize results.
...
The `SpatialStatistics` function can be used to count trees within a set of spatial units, as well as compute statistics of the trees' attributes. These spatial units can be in the form of spatial polygons, or can be generated in the form of a raster grid.

When no specific area is defined, `SpatialStatistics` will simply return the count of all inputted trees.
```{r}
SpatialStatistics(ttops)
```

Tree crowns can also be used as input. By defining the `variables` argument, `SpatialStatistics` will generate summarized statistics of the trees' attributes. By default, the mean, median, standard deviation, minimum and maximum are computed, but [custom functions](#customFunctions) can also be used.
...
Instead of defining polygonal areas, the `SpatialStatistics` function can also generate counts and stastics in raster format. In this case, the `grid` argument should be used instead of `areas`.
...
If, in addition to tree count, tree attribute statistics are computed, the object returned by `SpatialStatistics` will be a [RasterBrick](https://cran.r-project.org/package=raster/raster.pdf#page=36), i.e.: a multi-layered raster.
...
By default, the statistics generated by `SpatialStatistics` for each attribute will be its mean, median, standard deviation, minimum and maximum. However, by using the `statFuns` argument, custom functions can be used instead. 
...
To have this function applied using `SpatialStatistics`, it must be put into a named list. Naming the functions in the list is needed for labelling the function's outputs.
